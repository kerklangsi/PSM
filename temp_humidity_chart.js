...
function updateTempHumiChart() {
  ...
  if (['1m','10m','30m','max'].includes(timeRangeTempHumi)) {
    ...
    const tempAgg = aggregateData(filtered.map(p => ({ time: p.time, value: p.temp })), interval);
    const humiAgg = aggregateData(filtered.map(p => ({ time: p.time, value: p.humi })), interval);
    tempPoints = tempAgg;
    humiPoints = humiAgg;
  }
